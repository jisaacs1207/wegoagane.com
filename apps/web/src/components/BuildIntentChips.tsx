...
export function BuildIntentChips({ storageKey, onGenerate, isGenerating = false, hasGenerated = false }: Props) {
  ...
  function persistDepth(nextDepth: IntentDepth) {
    ...
    if (nextDepth === "dialed_in") setShowAdvanced(true);
  }
  ...
  const eventContext = useMemo(
    ...
    [activeIds.length, depth, showAdvanced, storageKey, value.buildVectors?.length, value.professionIntents?.length, value.raceMode, value.statPhilosophy?.length],
  );
  ...
  return (
    <div className="build-intent card" style={{ marginTop: 12 }}>
      ...
      <p className="hero-sub" style={{ marginTop: 0, marginBottom: 10, fontSize: 12 }}>
        Choose a depth, then generate. You can switch any time and tune before reroll.
      </p>
      ...
      <details style={{ marginBottom: 12 }}>
        ...
        <p className="hero-sub" style={{ marginTop: 8, marginBottom: 0, fontSize: 12 }}>
          Quick asks only core preference. Balanced keeps core plus optional advanced. Dialed-in opens advanced by
          default for tighter first-pass fit.
        </p>
      </details>
      ...
      <div className="flow-nav" style={{ marginBottom: 10 }}>
        <button
          type="button"
          className="btn-primary"
          disabled={isGenerating}
          onClick={() => {
            trackEvent(hasGenerated ? AnalyticsEvent.IntentRegenerateClicked : AnalyticsEvent.IntentGenerateClicked, eventContext);
        ...
        <button
          ...
          onClick={() => {
            ...
            trackEvent(AnalyticsEvent.IntentAdvancedToggled, { ...eventContext, open: next });
          }}
        >
          {showAdvanced ? "Hide advanced" : "Show advanced build focus"}
        </button>
      </div>
      {(showAdvanced || depth === "dialed_in") ? (
        <>
